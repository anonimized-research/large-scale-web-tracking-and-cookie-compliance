...
def process_web(web_id, web_url, results_path, error_path, cookies_url_path, beacons_path, fingerprinting_domains):
    ...
    try:
        ...
        if os.path.exists(results_file):
            ...
            # Previous error, try to process again to see if it works.
            previous_error = True
        ...
        # Process WEC with ACCEPT COOKIES.
        command = "website-evidence-collector "+full_web_url+" --max 1 --page-timeout 5000 --headless true --accept-cookies true --output "+wec_out_dir+" 2> /dev/null"
        ...
        # Get cookie acceptation.
        cookies_accepted = "1" if data["accepted_cookies"] else "0"
        cookies_accepted_text = '"'+data["accepted_cookies_text"]+'"' if data["accepted_cookies_text"] else '""'
        ...
        append_to_file(results_file, protocol_text+','+str(n_cookies)+','+str(n_beacons)+','+use_fingerprinting+','+cookies_accepted+','+cookies_accepted_text+'\n')
        with file_lock:
        ...
        # Indicate error (only if it was not previous error).
...
if __name__ == '__main__':
    ...
    # Web results path.
    results_path = 'results_accept'
    error_path = 'errors_accept'
    cookies_url_path = 'results_cookies_accept'
    beacons_path = 'results_beacons_accept'
